...
{
    public class OrderFormatterImpl : iOrderFormatter
    {
        public OrderFormatterImpl()
        {
        ...
        public IEnumerable<ModPair> GetFormattedModPairs(List<Mod> ModsFromSave, List<Mod> ModsFromActive)
        {
            ...
            while(SaveCurrent != null && ActiveCurrent != null) // 여기서 요소가 있다는 것을 보장함, 대신 다 끝나고 1개 또는 0개가 남을 수 있음
            {
                if(SaveCurrent.ModName == ActiveCurrent.ModName) // 동일할경우(그냥 추가하면 됨)
                {
                    ...
                    SaveCurrent = ModsFromSave.PopOrNull();
                    ActiveCurrent = ModsFromActive.PopOrNull();
                    continue;
                }
